	.data
main_msg:	.asciiz "Welcome to Caesar's cipher!\nCaesar's cipher is an encryption system that relies on a key value(n) to shift a message by n positions.\nThis is a slightly modified version were instead of a key value we use a key character contained in the latin alphabet, this key  character \nwill have a specific value based on its position in the alphabet (A or a = 0,B or b = 1, ..., Z or z = 25).\nOnce both the text message and the key value are available we shift the message by  applying the value of the key character, thus obtaining the encrypted or decrypted text!\n"
txt_en_or_dec:	.asciiz "Please enter (1):for the encryption function, (2):for the decryption function or (0):to end the program.\n"
txt_phrs_len:	.asciiz "How long do you think will your text be?\nEnter a value:"
txt_to_ciph:	.asciiz "Enter your message:"
txt_key:	.asciiz "Enter a key:"
txt_ciphered:	.asciiz "The new message is:"
...
main:
	...
	restart_prg:
	#txt select enryption or decryption
	la $a0, txt_en_or_dec
	syscall
	li $v0, 5
	syscall
	move $s4, $v0# s4 = de/encry command
	#1-->encryption #2-->decryption #0-->jump to end
	beq $s4, 0, end_prg
...
#a1 = key_value#key_letter
#a2 =  1( encrypt), a2 = 2 (decrypt)
#Output:
...
cipher:
	...
	#if v1== -1 then skip and store the special character
	move $s4, $a1
	...
	beq $s7, 1, encrypt_func
	sub $s4, $v1, $s2
	
	j decr_func
	encrypt_func:
	add $s4, $v1, $s2#encryption = char_val(text[i]) + key_value
	decr_func:
	div $s4, $t3
	mfhi $s4
